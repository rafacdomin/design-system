...
- **Gerenciamento de Contexto**: Contexto React local (`LocaleContext`, `LocaleProvider` e `useLocale`) no pacote `@rafacdomin/ds-docs`.
...
### 3.1 `@rafacdomin/ds-core` - Componente `Tag`
...
### 3.2 `@rafacdomin/ds-carousel` - Componente `Carousel`
...
## 4. Infraestrutura de i18n no Storybook (`@rafacdomin/ds-docs`)

Toda a complexidade de i18n reside no pacote de documentação:

### 4.1 Release Core (`.github/workflows/release-core.yml`)

Responsável por validar e publicar o pacote `@rafacdomin/ds-core`.
...
- **Etapas da Pipeline:**
  ...
  2. **Test:** Execução dos testes unitários e de acessibilidade via `pnpm --filter @rafacdomin/ds-core test` e `pnpm --filter @rafacdomin/ds-core lint`.
  3. **Visual Regression Test:** Build estática do Storybook e execução dos testes do Playwright (`pnpm --filter @rafacdomin/ds-docs test:visual`, que roda no Browserstack se as credenciais estiverem configuradas nos segredos do repositório, ou localmente caso contrário).
  4. **Bump Version:** Incrementa a versão do pacote no `package.json` de acordo com a seleção (ex: `pnpm --filter @rafacdomin/ds-core version ${{ github.event.inputs.version_increment }} --no-git-tag-version`).
  5. **Build:** Compilação dos componentes do `@rafacdomin/ds-core` para distribuição pública (ESM/CJS).
  6. **Publication:** Publicação no NPM (`pnpm --filter @rafacdomin/ds-core publish --no-git-checks --access public`) autenticada por meio da variável `NPM_TOKEN`.
...
Responsável por validar e publicar o pacote `@rafacdomin/ds-carousel`.
...
- **Etapas da Pipeline:**
  ...
  2. **Test:** Execução de testes unitários do `@rafacdomin/ds-carousel` e linter.
  3. **Visual Regression Test:** Execução dos testes visuais do Playwright para os stories do carrossel.
  4. **Bump Version:** Incrementa a versão do pacote no `package.json` de acordo com a seleção (ex: `pnpm --filter @rafacdomin/ds-carousel version ${{ github.event.inputs.version_increment }} --no-git-tag-version`).
  5. **Build:** Compilação da build de distribuição do `@rafacdomin/ds-carousel`.
...
- **Etapas da Pipeline:**
  ...
  2. **Build:** Geração da build estática de todo o monorepo (`pnpm build`) para garantir links de dependência, seguida por `pnpm --filter @rafacdomin/ds-docs build-storybook` para compilar o Storybook estático na pasta `packages/docs/storybook-static/`.
...
- **Etapas da Pipeline (Executadas em jobs paralelos):**
  ...
  - **Job `visual-tests` (Visual Regression):**
    ...
    3. Se presentes, realiza a compilação do Storybook e executa a suíte de testes de regressão visual do Playwright (`pnpm --filter @rafacdomin/ds-docs test:visual`).

---

## 5. Critérios de Done (Critérios de Conclusão)

1. Os componentes `@rafacdomin/ds-core` e `@rafacdomin/ds-carousel` não possuem referências, imports ou lógicas de internacionalização direta ou dicionários internos de tradução.
